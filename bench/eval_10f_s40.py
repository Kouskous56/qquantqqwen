"""10F s40 part2: V2 free + perm + GSM (PPL da co: 10.50)."""
import torch, re, json
from transformers import AutoModelForCausalLM, AutoTokenizer
from datasets import load_dataset
from collections import Counter
P = "D:/qwen/models/Qwen2.5-3B-FP16-pruned-s40"
...
print(f"s40 free {free}/50", flush=True)
letters, perq = [], {}
for cid, q, opts, cidx in Qs:
    for r in range(4):
...
print(f"s40 perm {perm}/200 Br {c['B']/200:.2f} cons {cons} rot {rot} dist {dict(c)}", flush=True)
...
for rr in ds:
    ...
    g += (extract(t) == exp)
print(f"s40 gsm {g}/50", flush=True)
json.dump({"s40": {"ppl": 10.50, "free": free, "perm": perm, "gsm": g,
                   "b_rate": round(c["B"] / 200, 3), "cons4": cons, "rot": rot}},
          open("D:/qwen/notes/RUN_10F_S40.json", "w"), indent=1)
print("saved RUN_10F_S40.json")
