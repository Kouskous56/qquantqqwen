...
P = "D:/qwen/models/Qwen2.5-3B-FP16-pruned-s50"
...
print(f"s50 free {free}/50", flush=True)
letters, perq = [], {}
for cid, q, opts, cidx in Qs:
    for r in range(4):
...
print(f"s50 perm {perm}/200 Br {c['B']/200:.2f} cons {cons} rot {rot} dist {dict(c)}", flush=True)
...
for rr in ds:
    ...
    g += (extract(t) == exp)
print(f"s50 gsm {g}/50", flush=True)
json.dump({"s40": {"ppl": 10.50, "free": free, "perm": perm, "gsm": g,
                   "b_rate": round(c["B"] / 200, 3), "cons4": cons, "rot": rot}},
          open("D:/qwen/notes/RUN_10F_S50.json", "w"), indent=1)
print("saved RUN_10F_S50.json")
